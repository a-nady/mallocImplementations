...
typedef struct {
    /* defined for both libc malloc and malloc package (mm.c) */
    ...
    /* defined only for the malloc package */
...
static int errors = 0;  /* number of errs found when running malloc */
...
int main(int argc, char **argv)
{
    ...
    stats_t *mm_stats = NULL;  /* mm stats for each trace */
 ...
 *     we've just called the mm_malloc to allocate a block of
...
static int eval_mm_valid(trace_t *trace, int tracenum, range_t **ranges) 
{
	...
	size = trace->ops[i].size;
	    ...
	    /* Remove region from list and call free function */
...
/* 
 * eval_mm_util - Evaluate the space utilization of the package
 */
static double eval_mm_util(trace_t *trace, int tracenum, range_t **ranges, size_t *min_space)
{   
    int i;
...
/*
 * eval_libc_valid - run this function to make sure that the
